<script>
    ...
    function beep() {
        const beepSound = new Audio('/audio/biosBeep.mp3'); // Replace <URL_TO_BEEP_SOUND> with the URL of your beep sound
        beepSound.play();
    }
    ...
    // Remove timer if user interacts by pressing up or down arrow key
...
<div class="w-screen h-screen bg-black overflow-hidden font-MSSS">
    <div class="mt-12 ml-8 text-lg">
        ...
        {#if !is_chromium}
            <p class="text-slate-100 mt-2 max-w-[500px]">RebornXP might have unexpected behaviors on browsers that are NOT Chromium-based (Safari, Firefox, Internet Explorer, etc.)</p>
        {/if}
