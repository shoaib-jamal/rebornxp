<script>
    ...
    onMount(async () => {
        ...
        if (utils.is_installing_windows()) {
            console.log("Dispatching load_page: /installation/95/installing");
            dispatcher("load_page", { url: "/installation/95/installing" });
        } else {
            console.log("Dispatching load_page: /xp/desktop");
            dispatcher("load_page", { url: "/xp/desktop" });
        }
    });
    ...
    function load_assets(assets, callback) {
        ...
        assets.forEach(asset => {
            const isImage = /\.(jpg|png|gif)$/i.test(asset);
            const isAudio = /\.(mp3)$/i.test(asset);
            const isFont = /\.(ttf)$/i.test(asset);
            ...
            if (isImage) {
                ...
                const font = new FontFace('temp', `url(${asset})`);
                font.load().then(() => {
                    loaded++;
                    if (loaded === total) callback();
                }).catch(() => {
                ...
                // For non-media files (e.g., .txt), just assume loaded
                loaded++;
                if (loaded === total) callback();
            }
        });
