<script>
    ...
    async function launch(program){
        ...
        if(path == './programs/my_computer.svelte'){
        ...
        } else if(path == './programs/pinball.svelte'){
            const Program = (await import('./programs/pinball.svelte')).default;
            let program = new Program({
                target: node_ref,
                props: {id: short.generate(), parentNode: node_ref, exec_path: path}
            });
        ...
        } else if(path == './programs/firefox.svelte'){
            const Program = (await import('./programs/firefox.svelte')).default;
            let program = new Program({
                target: node_ref,
                props: {id: short.generate(), parentNode: node_ref, exec_path: path}
            });
         ...
         } else if(path == './programs/collab.svelte'){
            const Program = (await import('./programs/collab.svelte')).default;
            let program = new Program({
                target: node_ref,
                props: {id: short.generate(), parentNode: node_ref, exec_path: path}
            });
