  ...
  onMount(async () => {
    ...
    while(true){
      ...
      if(time_passed >= total_duration){
        ...
        dispatcher('load_page', {url: './xp/starting/+page.svelte'});
        break;
      }
      ...
      if(current_phase.progress >= 100 && phase_index < phases.length){
        ...
        if(phase_index == 0){
          ...
          const ProductKeyWindows = (await import('../product_key_windows/+page.svelte')).default;
